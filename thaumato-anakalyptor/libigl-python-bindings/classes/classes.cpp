...
PYBIND11_MODULE(pyigl_classes, m)
{
  ...
  py::class_<igl::SLIMData>(m, "SLIM")
      // Constructor #1: last parameter is a double
      ...
      // Constructor #2: last parameter is a VectorXd
      .def(py::init([](Eigen::MatrixXd &v, 
                      Eigen::MatrixXi &f, 
                      Eigen::MatrixXd &v_init,
                      Eigen::VectorXi &b, 
                      Eigen::MatrixXd &bc,
                      int energy_type,
                      Eigen::VectorXd soft_penalty)
      {
        // The same shape checks:
        if (f.cols() == 3)
        {
          if (bc.cols() != 2)
            throw pybind11::value_error("bc must have shape (#bc, 2) for triangle mesh");
          if (v_init.cols() != 2)
            throw pybind11::value_error("v_init must have shape (#vertices, 2) for triangle mesh");
        }
        else if (f.cols() == 4)
        {
          if (bc.cols() != 3)
            throw pybind11::value_error("bc must have shape (#bc, 3) for tet mesh");
          if (v_init.cols() != 3)
            throw pybind11::value_error("v_init must have shape (#vertices, 3) for tet mesh");
        }
        else
        {
          throw pybind11::value_error("Unsupported element size in F");
        }

        if (energy_type >= igl::MappingEnergyType::NUM_SLIM_ENERGY_TYPES)
        {
          throw pybind11::value_error("Invalid energy_type");
        }

        auto sdata = std::make_unique<igl::SLIMData>();

        // Now call the VectorXd-based overload:
        igl::slim_precompute(v, f, v_init, *sdata, 
                            static_cast<igl::MappingEnergyType>(energy_type),
                            b, bc, soft_penalty);

        return sdata;
      }),
      py::arg("v"), py::arg("f"), py::arg("v_init"), 
      py::arg("b"), py::arg("bc"), py::arg("energy_type"), py::arg("soft_penalty"),
      R"pbdoc(
        Constructor that takes a vector of soft penalty values (one per dimension).
      )pbdoc"
      )
      .def(
          "set_energy_type", [](igl::SLIMData &self, int energy_type) {
          if (energy_type >= igl::MappingEnergyType::NUM_SLIM_ENERGY_TYPES)
          {
            throw pybind11::value_error("Invalid energy_type");
          }
          self.slim_energy = static_cast<igl::MappingEnergyType>(energy_type);
          },
          py::arg("energy_type"))
      .def(
