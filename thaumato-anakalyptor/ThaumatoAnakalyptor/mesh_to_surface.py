...
class MyPredictionWriter(BasePredictionWriter):
    def __init__(self, save_path, image_size, r, max_queue_size=10, max_workers=1, display=True, use_memmap=False, r_steps=None):
        ...
        self.image = None
        self.r = r
        # If r_steps is None, process all 2*r+1 layers; otherwise, process chunks of r_steps layers
        self.r_steps = r_steps  
        self.r_step_offset = 0  # current chunk offset (default 0)
    ...
    def write_on_batch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule, prediction, batch_indices, batch, batch_idx: int, dataloader_idx: int) -> None:
        ...
        if self.surface_volume_np is None:
            # Decide on the allocation shape based on whether we are processing a chunk
            if self.r_steps is not None:
                alloc_shape = (self.r_steps, self.image_size[0], self.image_size[1])
            else:
                alloc_shape = (2*self.r+1, self.image_size[0], self.image_size[1])
            if self.use_memmap:
                if trainer.global_rank == 0:
                    # Ensure output directory exists
                    os.makedirs(self.save_path, exist_ok=True)
                    # Create memmap on disk (only rank 0 creates it)
                    self.surface_volume_np = self.create_memmap_array(alloc_shape, np.uint16, filename="surface_volume_memmap.dat")
                    torch.distributed.barrier()  # Wait for other processes
                else:
                    torch.distributed.barrier()  # Wait until rank 0 creates the memmap
                    self.surface_volume_np = self.attach_memmap_array(alloc_shape, np.uint16, filename="surface_volume_memmap.dat")
            else:
                if trainer.global_rank == 0:
                    # Create a shared array and gather the shared memory name
                    self.surface_volume_np, self.shm = self.create_shared_array(alloc_shape, np.uint16, name="surface_volume")
                    torch.distributed.barrier()
                else:
                    torch.distributed.barrier()
                    self.surface_volume_np, self.shm = self.attach_shared_array(alloc_shape, np.uint16, name="surface_volume")
    ...
    def process_display_progress(self):
        ...
        try:
            # display progress using cv2.imshow
            image = (self.surface_volume_np[self.r_steps//2 if self.r_steps is not None else self.r].astype(np.float32) / 65535)
            screen_y = 2560
            screen_x = 1440
    ...
    def process_and_write_data(self, prediction):
        try:
            ...
            # If r_steps is specified, filter predictions to only include layers in the current chunk
            if self.r_steps is not None:
                mask = (indexes_3d[:, 0] >= self.r_step_offset) & (indexes_3d[:, 0] < self.r_step_offset + self.r_steps)
                if not np.any(mask):
                    return
                values = values[mask]
                indexes_3d = indexes_3d[mask]
                # Adjust layer index to be relative to the chunk
                indexes_3d[:, 0] -= self.r_step_offset

            # Save the predicted values into surface_volume_np
    ...
    def write_to_disk(self, flag='jpg'):
        ...
        if self.trainer_rank != 0:  # Only rank 0 should write to disk
            if not self.use_memmap:
                ...
                self.shm = None
            return
        # If processing in steps, write into a subfolder for this chunk
        out_path = self.save_path
        if self.r_steps is not None:
            out_path = os.path.join(self.save_path, f"chunk_{self.r_step_offset:03d}")
        ...
        if flag == 'tif':
            self.write_tif(out_path)
        elif flag == 'jpg':
            self.write_jpg(out_path)
        elif flag == 'memmap':
            self.write_memmap(out_path)
        elif flag == 'npz':
            self.write_npz(out_path)
        elif flag == 'zarr':
            self.write_zarr(out_path)
        else:
    ...
    def write_tif(self, out_path):
        ...
        with ThreadPoolExecutor(self.num_workers) as executor:
            futures = []
            total_layers = 2*self.r+1 if self.r_steps is None else 2*self.r+1
            for i in range(self.surface_volume_np.shape[0]):
                i_global = i + (self.r_step_offset if self.r_steps is not None else 0)
                i_str = str(i_global).zfill(len(str(total_layers)))
                filename = os.path.join(out_path, f"{i_str}.tif")
                futures.append(executor.submit(save_tif, i, filename))

            for future in tqdm(as_completed(futures), desc="Writing TIF"):
                future.result()

        # Create composite max image from all tifs in this chunk
        ...
        tifffile.imsave(os.path.join(os.path.dirname(out_path), f"composite_chunk_{self.r_step_offset:03d}.tif"), composite_image)

    def write_jpg(self, out_path, quality=60):  # Adjust the default quality value as needed
        ...
        composite_image = composite_image.T
        composite_image = composite_image[::-1, :]
        cv2.imwrite(os.path.join(os.path.dirname(out_path), f"composite_chunk_{self.r_step_offset:03d}.jpg"), composite_image, [cv2.IMWRITE_JPEG_QUALITY, quality])

    def write_memmap(self, out_path):
        memmap_path = os.path.join(out_path, "surface_volume")
        memmap = np.memmap(memmap_path, dtype='uint16', mode='w+', shape=self.surface_volume_np.shape)
        memmap[:] = self.surface_volume_np[:]
        del memmap

    def write_npz(self, out_path):
        npz_path = os.path.join(out_path, "surface_volume.npz")
        np.savez_compressed(npz_path, surface_volume=self.surface_volume_np)

    def write_zarr(self, out_path):
        ...
        zarr_path = os.path.join(out_path, "surface_volume.zarr")
...
class MeshDataset(Dataset):
    ...
    def load_grid_zarr(self, grid_index, uint8=False):
        grid_index_ = np.asarray(grid_index)[[2, 1, 0]]  # swap axis for zarr indexing. z x y
...
class PPMAndTextureModel(pl.LightningModule):
    ...
    def forward(self, x):
        # grid_coords: T x 3, grid_cell: B x W x H x D, vertices: T x 3 x 3, normals: T x 3 x 3, uv_coords_triangles: T x 3 x 2, grid_index: T
...
def ppm_and_texture(obj_path, scroll, output_path=None, grid_size=500, gpus=1, batch_size=1, r=32, format='jpg', max_side_triangle: int = 10, display=False, nr_workers=None, prefetch_factor=2, use_memmap=False, r_steps=None):
    # Number of workers
    ...
    total_layers = 2*r+1
    # If r_steps is not specified, process all layers at once
    if r_steps is None:
        writer.r_steps = None
        trainer.predict(model, dataloaders=dataloader, return_predictions=False)
        writer.write_to_disk(format)
    else:
        # Loop over chunks of r_steps
        for offset in range(0, total_layers, r_steps):
            writer.r_steps = min(r_steps, total_layers - offset)
            writer.r_step_offset = offset
            # Reset writer state before each pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('--r_steps', type=int, default=None, help="If specified, render this many layers at a time")
    args = parser.parse_known_args()[0]
    print(f"Rendering args: {args}")
    if args.display:
        print("[INFO]: Displaying the rendering image slows down the rendering process by about 20%.")
    ppm_and_texture(args.obj, gpus=args.gpus, scroll=args.scroll, output_path=args.output_path, r=args.r, format=args.format, max_side_triangle=args.max_side_triangle, display=args.display, nr_workers=args.nr_workers, prefetch_factor=args.prefetch_factor, use_memmap=args.memmap, r_steps=args.r_steps)
