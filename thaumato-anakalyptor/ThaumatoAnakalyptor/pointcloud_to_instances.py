...
def save_block_h5(h5f, block_points, block_normals, block_colors, block_scores, block_name, 
    ...
    for nr_surface in range(len(block_points)):
        ...
        surface_grp.create_dataset("points", data=block_points[nr_surface], compression="lzf", chunks=True) #, compression="gzip", compression_opts=8)
        surface_grp.create_dataset("normals", data=block_normals[nr_surface], compression="lzf", chunks=True) #, compression="gzip", compression_opts=8)
        surface_grp.create_dataset("colors", data=block_colors[nr_surface], compression="lzf", chunks=True) #, compression="gzip", compression_opts=8)
        surface_grp.create_dataset("coeffs", data=np.array(block_coeffs[nr_surface]), compression="lzf", chunks=True) #, compression="gzip", compression_opts=8)
