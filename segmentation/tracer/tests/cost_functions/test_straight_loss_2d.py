"""Tests for StraightLoss2D cost function."""
...
from cost_functions import StraightLoss2D


class TestStraightLoss2D(unittest.TestCase):
    """Test cases for StraightLoss2D."""
    # Allow more flexibility in our optimization tests
    tolerance = 0.1
    
    def test_straight_loss2d_error(self):
        """Test the error calculation for StraightLoss2D."""
        # Create optimization points - a straight line along the x-axis
        point_a = th.Point2(tensor=torch.tensor([[0.0, 0.0]]))
        point_b = th.Point2(tensor=torch.tensor([[1.0, 0.0]]))
        point_c = th.Point2(tensor=torch.tensor([[2.0, 0.0]]))
        
        # Cost weight
        cost_weight = th.ScaleCostWeight(1.0)
        
        # Create the cost function
        straight_loss = StraightLoss2D(point_a, point_b, point_c, cost_weight)
        ...
        point_b = th.Point2(tensor=torch.tensor([[1.0, 1.0]]))
        straight_loss = StraightLoss2D(point_a, point_b, point_c, cost_weight)
    ...
    def test_straight_loss2d_jacobians(self):
        """Test the Jacobian calculations for StraightLoss2D."""
        ...
        batch_size = 1
        expected_jac_b = torch.zeros(batch_size, 1, 2)
    ...
    def test_optimization(self):
        """Test that we can optimize a simple problem with StraightLoss2D."""
        ...
        point_b = th.Point2(tensor=torch.tensor([[1.0, 1.0]]), name="point_b")  # out of line
        point_c = th.Point2(tensor=torch.tensor([[2.0, 0.0]]), name="point_c")
        ...
        inputs = {
            ...
            "point_c": torch.tensor([[2.0, 0.0]])
        }
            
        # Optimize - this will only optimize point_b as it's the only one we registered
        # as an optimization variable in the StraightLoss2D constructor
        with torch.no_grad():
        ...
        b_tensor = th.Point2(tensor=optimized_point_b)
        optimized_loss = StraightLoss2D(point_a, b_tensor, point_c, cost_weight)
