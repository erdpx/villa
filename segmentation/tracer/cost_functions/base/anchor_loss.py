...
class AnchorLoss(th.CostFunction):
    ...
    - Interpolation is performed using (z, y, x) coordinates
    - This matches the C++ implementation's coordinate ordering
    """
    def __init__(
        self,
    ...
    def error(self) -> torch.Tensor:
        ...
        points = self.point.tensor            # Should be shape (batch_size, 3) in ZYX order
        anchor_points = self.anchor_point.tensor  # Should be shape (batch_size, 3) in ZYX order
        
        # Use the actual batch size from the tensors
        # Ensure both tensors have the same batch size
        if points.shape[0] != anchor_points.shape[0]:
            # If one has batch=1, broadcast it
            if points.shape[0] == 1:
                points = points.expand(anchor_points.shape[0], 3)
            elif anchor_points.shape[0] == 1:
                anchor_points = anchor_points.expand(points.shape[0], 3)
            else:
                # Different batch sizes that aren't 1 - use the smaller batch size
                min_batch = min(points.shape[0], anchor_points.shape[0])
                points = points[:min_batch]
                anchor_points = anchor_points[:min_batch]
        ...
        # Note: anchor_points tensor is already in ZYX order [z, y, x]
        z = anchor_points[:, 0:1]  # Z coordinate (first element in ZYX)
        y = anchor_points[:, 1:2]  # Y coordinate (second element in ZYX) 
        x = anchor_points[:, 2:3]  # X coordinate (third element in ZYX)
    ...
    def jacobians(self) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        # Get point coordinates
        points = self.point.tensor            # Should be shape (batch_size, 3)
        anchor_points = self.anchor_point.tensor  # Should be shape (batch_size, 3)
        ...
        # Note: gradients will be returned in ZYX order (matching our coordinate convention)
