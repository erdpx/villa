"""DistLoss2D cost function for theseus."""
...
class DistLoss2D(th.CostFunction):
    """
    A cost function that penalizes deviations from a target distance in 2D.
    
    This is a reimplementation of the DistLoss2D C++ cost function from 
    volume-cartographer. It penalizes the distance between two 2D points.
    ...
    def __init__(
        ...
        """
        Initialize the DistLoss2D cost function.
        ...
        if target_dist == 0:
            raise ValueError("target_dist can't be zero for DistLoss2D")
    ...
    def error(self) -> torch.Tensor:
        ...
        # If dist < target_dist, residual = w * (target_dist/(dist+1e-2) - 1)
        ...
        residual[close_dist_mask] = target_dist_tensor[close_dist_mask] / (dist[close_dist_mask] + 1e-2) - 1
    ...
    def jacobians(self) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        jac_a = torch.zeros(batch_size, 1, 2, device=a_tensor.device, dtype=a_tensor.dtype)
        ...
        if torch.any(close_dist_mask):
            for i in range(batch_size):
                if close_dist_mask[i, 0]:
                    ...
                    # The numerical constant is just the derivative of target_dist/(dist+1e-2) - 1
                    scale_close = self.target_dist / ((dist[i, 0] + 1e-2) ** 2) / torch.clamp(dist[i, 0], min=1e-6)
        ...
        if torch.any(far_dist_mask):
            for i in range(batch_size):
                if far_dist_mask[i, 0]:
                    ...
                    # For diff = a - b, moving a toward b means moving in direction of -diff
                    # The numerical constant is just the derivative of dist/target_dist - 1
                    scale_far = 1.0 / (self.target_dist * dist[i, 0])
                    jac_a[i, 0, :] = -scale_far * diff[i]  # Move point_a toward point_b (opposite of diff)
                    jac_b[i, 0, :] = scale_far * diff[i]   # Move point_b toward point_a (same as diff)
    ...
    def _copy_impl(self, new_name: Optional[str] = None) -> "DistLoss2D":
        """
