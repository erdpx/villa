...
class SpaceLineLossAcc(th.CostFunction):
    ...
    def _sample_line_points(self, point_a: torch.Tensor, point_b: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Args:
            point_a: First endpoint tensor of shape (batch_size, 3) in ZYX order [z, y, x]
            point_b: Second endpoint tensor of shape (batch_size, 3) in ZYX order [z, y, x]
            
        Returns:
            Tuple of (z, y, x) coordinates for sampled points
            Each has shape (batch_size, steps-1)
            The coordinate order matches requirements for interpolator.evaluate()
        """
        ...
        for b in range(batch_size):
            for i, f in enumerate(fractions):
                ...
                # Using ZYX coordinate convention: point[0]=z, point[1]=y, point[2]=x
                z_samples[b, i] = (1.0 - f) * point_a[b, 0] + f * point_b[b, 0]
                y_samples[b, i] = (1.0 - f) * point_a[b, 1] + f * point_b[b, 1]
                x_samples[b, i] = (1.0 - f) * point_a[b, 2] + f * point_b[b, 2]
    ...
    def error(self) -> torch.Tensor:
        ...
        for i in range(self.steps - 1):
            ...
            # Sample the volume at this point - passing coordinates in ZYX order
    ...
    def jacobians(self) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        for i, f in enumerate(fractions):
            ...
            # Get gradients at this point - passing coordinates in ZYX order
            # Gradients will also be returned in ZYX order
