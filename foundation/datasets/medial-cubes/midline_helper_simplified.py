...
def filter_and_reassign_labels(label_data, cc_min_size):
    ...
    for label in unique_labels:
        ...
        labels_out = cc3d.dust(label_mask, threshold=512, connectivity=26, in_place=False)
        labels_out = (labels_out > 0).astype(np.uint8)
        if np.sum(labels_out > 0):
            new_label_data[labels_out == 1] = new_label
            new_label += 1
...
def calculate_orientation(array_3d, label_value):
    ...
    if coordinates.shape[0] <= 3 or coordinates.shape[1] < 3:
        return False
